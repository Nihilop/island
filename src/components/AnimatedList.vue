...
<template>
    <div :class="cn('relative w-full', $props.class)">
        <div ref="listRef" class="max-h-100 overflow-y-auto p-4" :class="{ 'scrollbar-none': !displayScrollbar }"
            @scroll="handleScroll">
            <div v-for="(item, index) in items" :key="index" :ref="(el) => setItemRef(el as HTMLElement, index)"
                :data-index="index" class="mb-4 cursor-pointer transition-all duration-200" :style="{
                    transform: itemVisible[index] ? 'scale(1)' : 'scale(0.7)',
                    opacity: itemVisible[index] ? 1 : 0,
                    transitionDelay: '100ms',
                }" @mouseenter="selectedIndex = index" @click="handleItemClick(item, index)">
                <div class="rounded-lg p-4 transition-colors" :class="selectedIndex === index ? 'bg-muted-foreground/20' : 'bg-muted/50'
                    ">
        ...
        <template v-if="showGradients">
            <div class="pointer-events-none absolute inset-x-0 top-0 h-12.5 bg-linear-to-b from-background to-transparent transition-opacity duration-300"
